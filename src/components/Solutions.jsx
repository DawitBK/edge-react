...
const Solutions = () => {
  ...
  const imageVariant = (delay) => ({
    hidden: { opacity: 0, x: 100, scale: 0.8, rotate: 5 },
    visible: {
      opacity: 1,
      x: 0,
      scale: 1,
    ...
    <section id="solutions" className="bg-white py-32">
      <div className="max-w-7xl mx-auto w-[80%] space-y-32">
        ...
        <div className="space-y-32">
          {solutions.map((sol, idx) => {
            const delay = idx * 0.4; // sync delay for text + image
            return (
              <div
                key={idx}
                className={`grid md:grid-cols-2 gap-12 items-center ${
              ...
              >
                <motion.img
                  src={sol.img}
                  alt={sol.title}
                  className="w-full h-96 object-cover rounded-2xl shadow-2xl border-4 border-[#00BFFF]"
                  variants={imageVariant(delay)}
                  initial="hidden"
                  whileInView="visible"
